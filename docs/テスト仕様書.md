...
| SET-004 | 通知時間フォーマット不正 | ログイン済み | line_morning_at が "25:00" など不正（Accept: application/json） | 422 バリデーションエラー | 中 |
...
**通知設定（SET-003, SET-004）のテスト方法**

- **自動テスト**: `php artisan test tests/Feature/SettingsNotificationsTest.php`
  - 前提: PostgreSQL が起動していること（`compose up -d` 等）。テスト用 DB を `DB_DATABASE=testing` で作成しておく。
- **手動テスト（ブラウザ）**:
  1. 設定画面にログインし、通知設定の「保存」をクリック
  2. 開発者ツール（F12）→ Network タブを開く
  3. `<select name="line_morning_at">` を DevTools の Elements で編集し、`<option value="25:00">25:00</option>` を追加して選択、または `document.querySelector('[name="line_morning_at"]').value = '25:00'` で不正値をセット
  4. 保存ボタンをクリック → 422 応答とトーストで「入力内容をご確認ください」等のエラーが表示されることを確認
- **手動テスト（curl）**:
  ```bash
  # ログインしてセッションCookieを取得後
  curl -X PUT -H "Accept: application/json" -H "Cookie: ..." \
    -d "line_morning_at=25:00&line_evening_at=20:00&line_notify_enabled=0&_token=..." \
    http://localhost:8000/settings/notifications
  # → 422 と {"message":"...","errors":{"line_morning_at":[...]}} が返る
  ```
